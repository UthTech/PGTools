...
static void SetNodes( vector<pair<int, char*> >& nodeData, HWND hTree, HTREEITEM hParent, File* input)
{
	...
	while (!input->eof())
	{
		...
		stringstream str;
		str << hex << setw(8) << setfill('0') << chunk.getType() << "h (" << setw(8) << chunk.getSize() << "h)";
	    ...
	    item.item.pszText   = (char*)title.c_str();
...
static string FormatNodeInfo( int size, char* data, int width, const char* prefix = "")
{
	stringstream str;
	str << hex;
	for (int i = 0; i < size; i++)
	{
		...
		str << setw(2) << setfill('0') << (int)(unsigned char)data[i] << " ";
...
static void SetNodeInfo( HWND hWnd, int size, char* data, int width = 16 )
{
	...
	if (size < 0)
	{
		...
		while (size > 0)
		{
			...
			string chunk = FormatNodeInfo(msize, data + 2, width, "       ");
			if (chunk != "") chunk = chunk.substr(7);
			str << hex << setw(2) << setfill('0') << type  << " "
				       << setw(2) << setfill('0') << msize << ": " << chunk;
			size -= msize + 2;
			data += msize + 2;
		}
		text = str.str();
...
static void DlgOpenFile( ApplicationInfo *info )
{
	try
	{
		...
		ofn.lpstrFilter  = "Alamo Chunk Files (*.alo, *.ala, *.ted, *.tem, *.bui, *.rec)\0*.ALO; *.ALA; *.TED; *.TEM; *.BUI; *.REC\0All Files (*.*)\0*.*\0\0";
		ofn.nFilterIndex = 0;
	...
	{
		MessageBox(NULL, "Unable to open the specified file", NULL, MB_OK | MB_ICONHAND );
	}
}
...
static void RefreshFile(ApplicationInfo* info)
{
    ...
    {
        MessageBox(NULL, "Unable to refresh the opened file", NULL, MB_OK | MB_ICONHAND );
    }
}
...
static LRESULT CALLBACK MainWindowProc(HWND hWnd, UINT uMsg, WPARAM wParam, LPARAM lParam)
{
	...
	switch (uMsg)
	{
        case WM_CREATE:
        {
		    ...
		    if ((info->hNodeInfo = CreateWindowEx(WS_EX_CLIENTEDGE, "EDIT", "",
		...
		case WM_COMMAND:
			if (lParam == 0)
			{
				...
				switch (LOWORD(wParam))
				{
                    ...
                    case ID_HELP_ABOUT:
                        MessageBox(hWnd, "Alamo Chunk Viewer 1.0\n\nBy Mike Lankamp", "About", MB_OK);
                        break;
				}
			}
			else if (info != NULL)
			{
				...
				switch (HIWORD(wParam))
				{
					case EN_CHANGE:
					{
						...
						{
							int   width = strtoul(strWidth, 0, NULL);
...
static void CreateMainWindow( ApplicationInfo* info )
{
	...
	if ((info->hMainWnd = CreateWindowEx(0, "ChunkViewer", "Chunk File Viewer", WS_OVERLAPPEDWINDOW, 
		CW_USEDEFAULT, CW_USEDEFAULT, CW_USEDEFAULT, CW_USEDEFAULT, NULL, NULL, info->hInstance, info)) == NULL)
    {
        UnregisterClass("ChunkViewer", info->hInstance);
