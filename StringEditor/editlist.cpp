...
{
	LANGID language = (LANGID)(LONG_PTR)GetWindowLongPtr(hEdit, GWL_USERDATA);
...
{
	EDITLIST_INFO* info = (EDITLIST_INFO*)(LONG_PTR)GetWindowLongPtr(hWnd, GWL_USERDATA);
	switch (uMsg)
	{
		case WM_CREATE:
		{
...
BOOL CALLBACK DlgEntryProc(HWND hWnd, UINT uMsg, WPARAM wParam, LPARAM lParam)
{
	switch (uMsg)
	{
		case WM_INITDIALOG:
...
void EditList_AddPair(HWND hWnd, LANGID language, const wstring& postfix)
{
	...
	if (info != NULL)
	{
		...
		SetWindowLongPtr(GetDlgItem(ei.hWnd, IDC_EDIT1), GWL_USERDATA, (LONG)(LONG_PTR)language);
