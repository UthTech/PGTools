...
{
	IWindow* wnd = (IWindow*)(LONG_PTR)GetWindowLongPtr(hWnd, GWL_USERDATA);
	if (uMsg == WM_CREATE)
	{
		CREATESTRUCT* pcs = (CREATESTRUCT*)lParam;
		wnd = (IWindow*)pcs->lpCreateParams;
		SetWindowLongPtr(hWnd, GWL_USERDATA, (LONG)(LONG_PTR)wnd);
	}
...
static BOOL CALLBACK GenericDialogProc(HWND hWnd, UINT uMsg, WPARAM wParam, LPARAM lParam)
{
	IDialog* dlg = (IDialog*)(LONG_PTR)GetWindowLongPtr(hWnd, GWL_USERDATA);
	if (uMsg == WM_INITDIALOG)
	{
		dlg = (IDialog*)lParam;
		SetWindowLongPtr(hWnd, GWL_USERDATA, (LONG)(LONG_PTR)dlg);
	}
				...
				case TTN_GETDISPINFO:
				{
                    ...
                    for (int i = 0; Tooltips[i].idFrom != -1; i++)
                    {
                        if (Tooltips[i].idFrom == hdr->idFrom)
                        {
                            ...
                            nmdi->hinst    = (HINSTANCE)(LONG_PTR)GetWindowLongPtr(hWnd, GWL_HINSTANCE);
