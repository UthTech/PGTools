...
// New File
//
BOOL CALLBACK DlgNewFileProc(HWND hWnd, UINT uMsg, WPARAM wParam, LPARAM lParam)
{
	NEW_FILE_OPTIONS* info = (NEW_FILE_OPTIONS*)(LONG_PTR)GetWindowLongPtr(hWnd, GWL_USERDATA);
	switch (uMsg)
	{
		case WM_INITDIALOG:
		{
...
{
	return (DialogBoxParam((HINSTANCE)(LONG_PTR)GetWindowLongPtr(hParent, GWL_HINSTANCE), MAKEINTRESOURCE(IDD_NEWFILE), hParent, DlgNewFileProc, (LPARAM)&options) != 0);
}

//
// Import File
//
BOOL CALLBACK DlgImportProc(HWND hWnd, UINT uMsg, WPARAM wParam, LPARAM lParam)
{
	switch (uMsg)
	{
		case WM_INITDIALOG:
...
Document::Method ImportFile(HWND hParent, bool byIndex)
{
	...
	return (Document::Method)DialogBox((HINSTANCE)(LONG_PTR)GetWindowLongPtr(hParent, GWL_HINSTANCE), MAKEINTRESOURCE(dialog), hParent, DlgImportProc);
}

BOOL CALLBACK DlgVersionInfoProc(HWND hWnd, UINT uMsg, WPARAM wParam, LPARAM lParam)
{
	VERSION_INFO* info = (VERSION_INFO*)(LONG_PTR)GetWindowLongPtr(hWnd, GWL_USERDATA);
	switch (uMsg)
	{
		case WM_INITDIALOG:
		{
...
{
	return (DialogBoxParam((HINSTANCE)(LONG_PTR)GetWindowLongPtr(hParent, GWL_HINSTANCE), MAKEINTRESOURCE(IDD_VERSION_INFO), hParent, DlgVersionInfoProc, (LPARAM)&info) != 0);
}
...
BOOL CALLBACK DlgLangSelectProc(HWND hWnd, UINT uMsg, WPARAM wParam, LPARAM lParam)
{
	LANGUAGE_SELECT* info = (LANGUAGE_SELECT*)(LONG_PTR)GetWindowLongPtr(hWnd, GWL_USERDATA);
	switch (uMsg)
	{
		case WM_INITDIALOG:
		{
...
LANGID AddLanguage(HWND hParent, const std::set<LANGID>& languages)
{
	...
	return (LANGID)DialogBoxParam((HINSTANCE)(LONG_PTR)GetWindowLongPtr(hParent, GWL_HINSTANCE), MAKEINTRESOURCE(IDD_LANG_SELECT), hParent, DlgLangSelectProc, (LPARAM)&langselect);
}
...
BOOL CALLBACK DlgExportProc(HWND hWnd, UINT uMsg, WPARAM wParam, LPARAM lParam)
{
	EXPORT_INFO* info = (EXPORT_INFO*)(LONG_PTR)GetWindowLongPtr(hWnd, GWL_USERDATA);
	switch (uMsg)
	{
		case WM_INITDIALOG:
		{
...
bool ExportFile(HWND hParent, map<LANGID,wstring>& postfixes, map<LANGID,bool>& enabled, wstring& basename)
{
	...
	return (DialogBoxParam((HINSTANCE)(LONG_PTR)GetWindowLongPtr(hParent, GWL_HINSTANCE), MAKEINTRESOURCE(IDD_EXPORT), hParent, DlgExportProc, (LPARAM)&info) != 0);
}

BOOL CALLBACK DlgFindProc(HWND hWnd, UINT uMsg, WPARAM wParam, LPARAM lParam)
{
	FIND_INFO* info = (FIND_INFO*)(LONG_PTR)GetWindowLongPtr(hWnd, GWL_USERDATA);
	switch (uMsg)
	{
		case WM_INITDIALOG:
...
{
	return (DialogBoxParam((HINSTANCE)(LONG_PTR)GetWindowLongPtr(hParent, GWL_HINSTANCE), MAKEINTRESOURCE(IDD_FIND), hParent, DlgFindProc, (LPARAM)&info) != 0);
}

//
// File details
//
BOOL CALLBACK DlgFileInfoProc(HWND hWnd, UINT uMsg, WPARAM wParam, LPARAM lParam)
{
	wchar_t tmp[32];

	const Document* document = (const Document*)(LONG_PTR)GetWindowLongPtr(hWnd, GWL_USERDATA);
	switch (uMsg)
	{
		case WM_INITDIALOG:
		{
			document = (const Document*)lParam;
			SetWindowLongPtr(hWnd, GWL_USERDATA, (LONG)(LONG_PTR)document);
...
{
	DialogBoxParam((HINSTANCE)(LONG_PTR)GetWindowLongPtr(hParent, GWL_HINSTANCE), MAKEINTRESOURCE(IDD_FILE_INFO), hParent, DlgFileInfoProc, (LPARAM)document);
}
}
