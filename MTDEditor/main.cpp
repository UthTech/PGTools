...
static bool DoSaveFile( ApplicationInfo* info, bool saveas = false )
{
	if (info->openfile->isUnnamed() || saveas)
	{
		...
		TCHAR filename1[MAX_PATH]; wcsncpy(filename1, info->openfile->getIndexFilename().c_str(), MAX_PATH );
		TCHAR filename2[MAX_PATH]; wcsncpy(filename2, info->openfile->getImageFilename().c_str(), MAX_PATH );
...
BOOL CALLBACK MainWindowProc(HWND hWnd, UINT uMsg, WPARAM wParam, LPARAM lParam)
{
	...
	switch (uMsg)
	{
        case WM_INITDIALOG:
        {
            ...
            SetWindowLongPtr(info->hRenderWnd, GWLP_USERDATA, (LONG)(LONG_PTR)info);
            break;
        }
